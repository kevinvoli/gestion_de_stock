...
from controller import json_verify, ControllerEntite
    ...
    try:
        ...
        if is_verfy is True:
            if entite in ['notification']:
                ...
                response =  {"satus": 'Failed', 'message': f'Le service {entite} ne peut communiquer avec le service Notification'}
                response_security = security.register_failure(ip)
                logger.error(f"⛔ Accès refusé - Requête ID: {id}, Entité invalide - {entite}")
        else:
            response = {'erreur': f'Le service {service_name} ne peut communiquer avec le service Notification'}
            response_security = security.register_failure(ip)
            logger.error(f"⛔ Accès refusé - Requête ID: {id}, Clé API invalide - {api_key}")
...
async def server_tcp_notif():
    asyncio.create_task(security.cleanup_blocked_ips()) # Supprime les addresse ip bloquée
    # Création du serveur TCP
    server = await asyncio.start_server(
        handle_client, host, 3005  # Adresse et port du serveur
    )
